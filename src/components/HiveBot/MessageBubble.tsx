...
export default function MessageBubble({ item, onContentLoad }: MessageBubbleProps) {
  ...
  return (
    <div className={clsx("flex gap-3 group", isUser ? "justify-end" : "justify-start")}>
      {!isUser && (
      ...
      <div className={clsx("flex flex-col gap-1 max-w-[85%] sm:max-w-[75%]", isUser && "items-end")}>
        {item.fileDataUrl && (
          <div className="relative w-48 sm:w-64 aspect-video rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm">
            <Image
